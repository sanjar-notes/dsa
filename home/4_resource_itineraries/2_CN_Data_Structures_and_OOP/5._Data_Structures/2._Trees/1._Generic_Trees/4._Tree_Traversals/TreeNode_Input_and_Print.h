...
public:
    ...
    ~TreeNode()
    {
        ...
        for (int i = 0; i < children.size(); i++)
            delete children.at(i); // that also is a Node, again the destructor is called
        // leaves are deleted
        // recursive calls
        cout << "Node deleted\n";
    }
...
template <typename T>
int numNodesLeaves(TreeNode<T> *root)
{
    if (root == NULL) // an edge case
        return 0;

    // base case - controlled
    if (root->children.size() == 0)
        return 1;

    int leaves = 0;
    for (int i = 0; i < root->children.size(); i++)
        leaves += numNodesLeaves(root->children.at(i));
